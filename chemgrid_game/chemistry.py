...
def generate_random_mol(seed: int, n_atoms: int, n_colors: int = 1, max_size: int = 8) -> Molecule:
    rng = np.random.default_rng(seed)
    atoms = np.zeros((max_size, max_size), dtype=np.uint8)
    i = rng.integers(max_size * max_size)
    x, y = np.unravel_index(i, (max_size, max_size))
    atoms[x, y] = 1
    ...
    return Molecule(atoms, max_size=max_size)


def create_unit_mol(col_id: int, max_size=8) -> Molecule:
    assert 1 <= col_id <= 3
    atoms = np.zeros((max_size, max_size), dtype=np.uint8)
    atoms[0, 0] = col_id
    return Molecule(atoms, max_size=max_size)
