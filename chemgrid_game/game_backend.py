...
class GameBackend:
    ...
    def check_contracts(self):
        ...
        for agent_id in range(self.n_agents):
            q.extend((agent_id, mol_id) for mol_id in self.inventories[agent_id])

        while len(q) > 0:
            agent_id, mol_id = q.popleft()
            ...
            for other_agent_id, offer_id, ask_id in self.contracts:
                ...
                if ask_id == mol_id and offer_id in other_inventory:
                    if offer_id not in inventory:
                        inventory.append(offer_id)
                        q.append((agent_id, offer_id))

                    if ask_id not in other_inventory:
                        other_inventory.append(ask_id)
                        q.append((other_agent_id, ask_id))

                if offer_id == mol_id and ask_id in other_inventory:
                    if ask_id not in inventory:
                        inventory.append(ask_id)
                        q.append((agent_id, ask_id))

                    if offer_id not in other_inventory:
                        other_inventory.append(offer_id)
                        q.append((other_agent_id, offer_id))

    def get_state(self, agent_id: int) -> State:
    ...
    def get_states(self) -> Tuple[State]:
        return tuple([self.get_state(i) for i in range(self.n_agents)])
    ...
    def add_mol(self, agent_id: int, mol: Molecule):
        mol_id = hash(mol)
